...
class ExchangeExternalSignatures(private val e2e: E2ETest) {
    ...
    private val openbadgeSdJwtIssuanceRequest =
        Json.decodeFromJsonElement<IssuanceRequest>(WaltidServicesIntegrationTests.sdjwtCredential).apply {
    ...
    init {
        client = environment.testHttpClient()
    ...
    private suspend fun combinedW3CIETFSdJwtVCTestCase() {
        ...
        val openbadgeSdJwtIssuanceRequest =
            Json.decodeFromJsonElement<IssuanceRequest>(WaltidServicesIntegrationTests.sdjwtCredential).apply {
                credentialFormat = CredentialFormat.jwt_vc_json
            }
        testPreAuthorizedOID4VCI(
