...
object LspPotentialIssuanceInterop {
  val POTENTIAL_ISSUER_KEY_JWK = ECKey.parseFromPEMEncodedObjects(LspPotentialInterop.POTENTIAL_ISSUER_PRIV + LspPotentialInterop.POTENTIAL_ISSUER_PUB).toJSONString()
  ...
  fun loadPotentialIssuerKeys(): COSECryptoProviderKeyInfo {
    ...
    val issuerPub = KeyFactory.getInstance("EC").generatePublic(X509EncodedKeySpec(readKeySpec(LspPotentialInterop.POTENTIAL_ISSUER_PUB)))
    val issuerPriv = KeyFactory.getInstance("EC").generatePrivate(PKCS8EncodedKeySpec(readKeySpec(LspPotentialInterop.POTENTIAL_ISSUER_PRIV)))
    return COSECryptoProviderKeyInfo(LspPotentialInterop.POTENTIAL_ISSUER_KEY_ID, AlgorithmID.ECDSA_256, issuerPub, issuerPriv, listOf(issuerCert), listOf(rootCaCert))
  }
}
