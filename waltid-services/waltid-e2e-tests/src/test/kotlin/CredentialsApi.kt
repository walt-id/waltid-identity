...
class CredentialsApi(private val client: HttpClient) {
    ...
    suspend fun restore(wallet: UUID, credential: String) =
    ...
    suspend fun reject(wallet: UUID, credential: String) =
        test("/wallet-api/wallet/{wallet}/credentials/{credentialId}/reject - reject credential") {
            client.post("/wallet-api/wallet/$wallet/credentials/$credential/reject") {
                setBody(mapOf("note" to ""))
            }.expectSuccess()
        }

    suspend fun status(wallet: UUID, credential: String) =
    ...
    suspend fun attachCategory(wallet: UUID, credential: String, categories: List<String>) =
    ...
    suspend fun detachCategory(wallet: UUID, credential: String, categories: List<String>) =
