...
class EBSI_Conformance_Test : AnnotationSpec() {
    ...
    fun testReceiveCredential() {
        val initCredentialOfferUrl =
            "https://api-conformance.ebsi.eu/conformance/v3/issuer-mock/initiate-credential-offer?credential_type=CTWalletCrossInTime&client_id=${credentialWallet.TEST_DID}&credential_offer_endpoint=openid-credential-offer://"
        ...
        val credentialResponses = credentialWallet.executeFullAuthIssuance(
            credentialOfferRequest,
            credentialWallet.TEST_DID,
            OpenIDClientConfig(credentialWallet.TEST_DID, null, credentialWallet.config.redirectUri, useCodeChallenge = true)
        )
