...
object OpenID4VCI {
  fun getCredentialOfferRequestUrl(credOffer: CredentialOffer, credentialOfferEndpoint: String = CROSS_DEVICE_CREDENTIAL_OFFER_URL, customReqParams: Map<String, List<String>> = mapOf()): String {
  ...
  fun getCredentialOfferRequestUrl(credOfferUrl: String, credentialOfferEndpoint: String = CROSS_DEVICE_CREDENTIAL_OFFER_URL, customReqParams: Map<String, List<String>> = mapOf()): String {
    return URLBuilder(credentialOfferEndpoint).apply {
      parameters.appendAll(parametersOf(CredentialOfferRequest(credentialOfferUri = credOfferUrl, customParameters = customReqParams).toHttpParameters()))
    }.buildString()
  }
  ...
  suspend fun parseAndResolveCredentialOfferRequestUrl(credOfferReqUrl: String): CredentialOffer {
    ...
    return if(offerReq.credentialOffer != null) {
      ...
      httpGet(offerReq.credentialOfferUri).let {
  ...
  suspend fun resolveCIProviderMetadata(credOffer: CredentialOffer): OpenIDProviderMetadata {
    return httpGet(getCIProviderMetadataUrl(credOffer)).let { OpenIDProviderMetadata.fromJSONString(it) }
  }
