...
actual class LocalKey actual constructor(
    ...
    actual companion object : KeyCreator {
        actual override suspend fun generate(type: KeyType, metadata: KeyMetadata): LocalKey = LocalKeyGenerator.generate(type, metadata)
        actual override suspend fun importJWK(jwk: String): Result<LocalKey> = LocalKeyGenerator.importJWK(jwk)
        actual override suspend fun importPEM(pem: String): Result<LocalKey> = LocalKeyGenerator.importPEM(pem)
        actual override suspend fun importRawPublicKey(type: KeyType, metadata: KeyMetadata, rawPublicKey: ByteArray): Key =
            LocalKeyGenerator.importRawPublicKey(type, metadata, rawPublicKey)
    }
}
