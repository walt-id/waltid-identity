...
class IosKeyTest {
    ...
    // TODO: Keychain tests require a real app bundle context with entitlements.
    //  Add an iOS test app to run these. Uncomment when available.
    //  private val platformKeyTypes = listOf(KeyType.secp256r1, KeyType.RSA)

    // --- Platform key store ---
    // These tests are correct but cannot run in the bare simulator test runner.
    // They mirror AndroidKeyTest's platform key store section.

    // @Test
    // fun platformCreate() = runTest {
    //     for (type in platformKeyTypes) {
    //         val key = IosKey.Platform.create(IosKey.Options(kid = "test-platform-$type", keyType = type))
    ...
    //     for (type in platformKeyTypes) {
    //         val key = IosKey.Platform.create(IosKey.Options(kid = "test-platform-sign-$type", keyType = type))
    ...
    //     for (type in platformKeyTypes) {
    //         val key = IosKey.Platform.create(IosKey.Options(kid = "test-platform-jws-$type", keyType = type))
    ...
    //     for (type in platformKeyTypes) {
    //         val key = IosKey.Platform.create(IosKey.Options(kid = "test-platform-jwk-$type", keyType = type))
    ...
    //     for (type in platformKeyTypes) {
    //         val key = IosKey.Platform.create(IosKey.Options(kid = "test-platform-thumb-$type", keyType = type))
