...
internal object EtsiProviderCertificateSupport {
     ...
     */
    fun X509CertificateDataBuilder.applyProviderCertificate(
        subjectKey: Key?,
        subjectDn: String,
        qcTypeOid: String,
    ...
    suspend fun validateProviderCertificate(
        context: ValidationContext,
        x509Certificate: X509Certificate,
        qcTypeOid: String,
    ) {
        IsoProfileX509CertificateValidationUtil.validateVersion(context, x509Certificate)
        IsoProfileX509CertificateValidationUtil.validateSerialNumber(context, x509Certificate)
    ...
    private fun validatePositiveValidity(context: ValidationContext, x509Certificate: X509Certificate) {
    ...
    fun validateEndEntity(context: ValidationContext, x509Certificate: X509Certificate) {
        ...
        if (extension == null) {
            ...
            if (!extension.critical) {
                context.addLogEntry(ValidationResult.Severity.ERROR, "basicConstraints", "Certificate extension 'basicConstraints' must have a critical flag set")
            }
            if (extension.cA) {
                context.addLogEntry(ValidationResult.Severity.ERROR, "basicConstraints", "Certificate must be an end-entity certificate (cA=FALSE)")
            }
        }
    }
    ...
    fun validateKeyUsage(context: ValidationContext, x509Certificate: X509Certificate) {
        ...
        if (extension == null) {
            ...
            if (!extension.critical) {
                context.addLogEntry(ValidationResult.Severity.ERROR, "keyUsage", "Certificate extension 'keyUsage' must have a critical flag set")
            }
    ...
    fun validateSubjectKeyIdentifier(context: ValidationContext, x509Certificate: X509Certificate) {
        ...
        if (extension == null) {
            ...
            context.addLogEntry(ValidationResult.Severity.ERROR, "subjectKeyIdentifier", "Certificate extension 'subjectKeyIdentifier' must not have a critical flag set")
        }
    }
    ...
    fun validateAuthorityInfoAccessIfCaIssued(context: ValidationContext, x509Certificate: X509Certificate) {
        ...
        if (extension == null || extension.accessDescriptions.isEmpty()) {
            context.addLogEntry(ValidationResult.Severity.ERROR, "authorityInfoAccess", "Certificate extension 'authorityInfoAccess' is required for CA-issued certificates")
        }
    }
    ...
    private fun validateQcStatements(context: ValidationContext, x509Certificate: X509Certificate, qcTypeOid: String) {
        ...
        if (extension == null) {
            context.addLogEntry(ValidationResult.Severity.ERROR, "qcStatements", "Certificate extension 'qcStatements' is not present")
            return
        }
    ...
    fun validatePersonDnByRole(context: ValidationContext, attribute: String, dnString: String, isLegalPerson: Boolean) {
        ...
        if (countryName.isNullOrEmpty()) {
            context.addLogEntry(ValidationResult.Severity.ERROR, "${attribute}Dn", "Missing countryName in $attribute DN")
        }
        ...
        if (isLegalPerson) {
            ...
            if (grouped["serialnumber"].isNullOrEmpty()) {
                context.addLogEntry(ValidationResult.Severity.ERROR, "${attribute}Dn", "Natural person $attribute DN is missing serialNumber")
            }
        }
    }
}
