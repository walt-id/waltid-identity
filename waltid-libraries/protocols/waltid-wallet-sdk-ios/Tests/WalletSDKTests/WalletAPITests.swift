...
final class WalletAPITests: XCTestCase {
    ...
    func testResolveOfferForwardsOfferAndReturnsRequirement() async throws {
        ...
        let bridge = FakeWalletCoreBridge()
        let transactionCode = TransactionCode(inputMode: .numeric, length: 6, description: "Enter the issuer code")
        bridge.offerResolutionResult = OfferResolution(transactionCode: transactionCode)
        let wallet = Wallet(bridge: bridge)

        let result = try await wallet.resolveOffer(offer: offer)

        XCTAssertEqual(result, OfferResolution(transactionCode: transactionCode))
