...
class ResponseEncryptionJvmTest {
    @Test
    fun encryptResponseRoundTripsPayload() = runTest {
        ...
        val config = requireNotNull(ResponseEncryption.resolve(request))
        val payload = buildJsonObject {
            put("vp_token", JsonPrimitive("credential-presentation"))
            put("state", JsonPrimitive("state-123"))
        }

        val jwe = ResponseEncryption.encryptResponse(payload, config)
