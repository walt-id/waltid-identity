...
`createIssuanceHolderKeys` returns `MobileWalletIssuanceHolderKey` entries and
does not replace the active signing identity. It defaults to P-256 and the
configured key-use authorization policy. When issuance requires a restricted
`SigningIdentityKeyPolicy`, all selections must keep the identity key validated
at session creation; alternate batch holder keys are rejected, including after restart.
