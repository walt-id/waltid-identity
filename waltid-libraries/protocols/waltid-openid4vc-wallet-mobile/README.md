...
`MobileWalletConfig()` does not accept any OpenID4VP `transaction_data` profiles by default. Wallet apps must pass the profile types they understand through `transactionDataProfiles`; requests containing unknown transaction data types are rejected before the user can submit a presentation. Profile fields are preserved for app UI and display metadata. `MobileWalletTransactionDataProfiles.all` is a ready-made catalog of walt.id demo and EUDI types; pass a subset when the wallet should accept fewer types.
