...
class LegacyRecoveryDerivationTest {
    @Test fun independentHmacAndOpenSslVectorMatches() = runTest {
        val derived = LegacyRecoveryDerivation.derive(ByteArray(32) { it.toByte() }, "wal-749-vector-1")
        ...
        assertNotEquals(derived, LegacyRecoveryDerivation.derive(ByteArray(32) { it.toByte() }, "other-domain"))
        assertNotEquals(derived, LegacyRecoveryDerivation.derive(ByteArray(32) { it.toByte() }, "wal-749-vector-1", 1))
    }

    @Test fun invalidInputsAreRejected() = runTest {
        for (length in listOf(0, 16, 31, 33, 64)) assertFailsWith<IllegalArgumentException> {
            LegacyRecoveryDerivation.derive(ByteArray(length), "test")
        }
        for (domain in listOf("", "contains space", "é", "x".repeat(129))) assertFailsWith<IllegalArgumentException> {
            LegacyRecoveryDerivation.derive(ByteArray(32), domain)
        }
        assertFailsWith<IllegalArgumentException> { LegacyRecoveryDerivation.derive(ByteArray(32), "test", -1) }
    }
}
