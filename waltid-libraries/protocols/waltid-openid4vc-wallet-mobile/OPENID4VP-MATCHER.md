...
| Binary | `target/wasm32-unknown-unknown/release/presentation.wasm` |
| Size | 117,546 bytes |
...
Built from unmodified upstream sources with upstream's own `build.sh`:
...
The build is **not** bit-reproducible across toolchains: `build.sh` uses `cargo +nightly` with
`-Z build-std`, so the exact bytes depend on the nightly in use. `AndroidVendoredMatcherTest` pins the
committed binary's hash so a swap cannot go unnoticed, and the hash above records which build the
committed bytes came from. Upstream publishes no release artifact for this binary, so there is nothing
to verify it against; what is verifiable is that the committed bytes are what this toolchain produces
from that commit, and that upstream's own 82-test suite passes at that commit.
...
Multipaz-private convention is involved. Three things still have to hold:
...
- **Host imports.** The matcher imports its host functions from module `credman`, whereas AndroidX's C
  ...
  AndroidX's.
- **Optional credential sets are dropped.** `evaluate_explicit_credential_sets` filters on
  `required.unwrap_or(true)` and never evaluates `required: false` sets, so a credential only an
  optional set asks for contributes no candidates. AndroidX's C matcher reports them. Nothing in this
  repo sends optional sets, and no test covers them; required sets are covered by
  `DigitalCredentialSharingE2ETest.sharesTheSatisfiableCredentialSetOption`. Treat this as the one
  known behaviour difference to check before sending an optional set to an Android holder.
