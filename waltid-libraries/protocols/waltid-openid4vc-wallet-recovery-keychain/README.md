...
Register `KeychainIdentityRecovery(namespace)` in `SigningIdentityConfiguration.recoveryProviders`. Signing keys and synchronizable recovery records use separate accessibility settings. Native Swift consumers can opt into the `WalletSDKKeychainRecovery` product instead.
