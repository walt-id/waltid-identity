 ...
 *
 * @see ISO/IEC DIS 18013-5:2026(en), 8.3.2.1.2.3 (Device retrieval mdoc response)
 *
...
data class IssuerSignedList(
    ...
    companion object {
         ...
         *
         * @see ISO/IEC DIS 18013-5:2026(en), 8.3.2.1.2.3 (CDDL for IssuerSignedItemBytes)
         *
        ...
        fun fromIssuerSignedItems(items: List<IssuerSignedItem>) =
            IssuerSignedList(items.map { item ->
                ByteStringWrapper(
                    item,
                    coseCompliantCbor.encodeToByteArray(IssuerSignedItem.serializer(), item)
                )
            })
