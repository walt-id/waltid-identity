    ...
    private fun collectPresentedTypes(data: JsonObject): List<String> {
        ...
        if (vcArray != null) {
            return vcArray.mapNotNull { vcEl ->
                ...
                val payload = runCatching { SDJwt.parse(sdJwtStr).fullPayload }.getOrNull()
                payload?.let {
                    it["vct"]?.jsonPrimitive?.contentOrNull
                        ?: it["vc"]?.jsonObject?.get("type")?.jsonArray?.lastOrNull()?.jsonPrimitive?.contentOrNull
                        ?: it["type"]?.jsonArray?.lastOrNull()?.jsonPrimitive?.contentOrNull
                }
            }
        }
