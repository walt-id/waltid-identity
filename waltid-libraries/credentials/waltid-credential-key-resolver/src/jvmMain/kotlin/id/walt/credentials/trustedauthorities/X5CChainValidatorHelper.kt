        ...
        for (i in 0 until certs.size - 1) {
            ...
            try {
                ...
                throw IllegalArgumentException(
                    "x5c chain validation failed at position $i: " +
                        "'${subject.subjectX500Principal}' is not signed by '${issuer.subjectX500Principal}': ${e.message}"
                )
            }
            ...
            if (subjectAki != null && issuerSki != null && !subjectAki.contentEquals(issuerSki)) {
                throw IllegalArgumentException(
                    "x5c chain AKI/SKI mismatch at position $i: " +
                        "AKI of '${subject.subjectX500Principal}' does not match SKI of '${issuer.subjectX500Principal}'"
                )
            }
        }
        log.trace { "x5c chain verification passed for ${certs.size} certificates" }
    }
}
