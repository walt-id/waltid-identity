...
fun Application.configureSecurity() {
    ...
    install(Authentication) {
        ...
        session<LoginTokenSession>("auth-session") {
            ...
            challenge {
                call.respond(
                    HttpStatusCode.Unauthorized, JsonObject(mapOf("message" to JsonPrimitive("Login Required")))
                )
            }
        }
    }
    ...
    webWalletRoute {
        route("auth", { tags = listOf("Authentication") }) {
            ...
            post("login", {
                ...
                AccountsService.authenticate("", reqBody).onSuccess { // FIXME -> TENANT HERE
                    ...
                    call.respond(
                        mapOf(
                            ...
                            "username" to it.username //TODO: change id to wallet-id (also in the frontend)
                        )
                    )
                }.onFailure {
                    it.printStackTrace()
            ...
            authenticate("auth-session", "auth-bearer", "auth-bearer-alternative") {
                ...
                get("session", { summary = "Return session ID if logged in" }) {
                    ...
                    if (securityUserTokenMapping.contains(token)) call.respond(mapOf("token" to mapOf("accessToken" to token)))
            ...
            route("keycloak", { tags = listOf("Keycloak Authentication") }) {

                // Generates Keycloak access token
                get("token", { summary = "Returns Keycloak access token" }) {
                ...
                post("create", {
                    ...
                    request {
                        body<KeycloakAccountRequest> {
                            example("Username + password", buildJsonObject {
                ...
                post("login", {
                    ...
                    request {
                        body<KeycloakAccountRequest> {
                            example("Keycloak username + password", buildJsonObject {
                                put("type", JsonPrimitive("keycloak"))
                                put("username", JsonPrimitive("Max_Mustermann"))
                                put("password", JsonPrimitive("password"))
                            }.toString())
                    ...
                    AccountsService.authenticate("", reqBody).onSuccess { // FIXME -> TENANT HERE
                        ...
                        call.respond(
                            mapOf(
                                "token" to it.token,
                                "id" to it.id.toString(),
                                "keycloakUserId" to it.username //TODO: change id to wallet-id (also in the frontend)
                            )
                        )
                    }.onFailure {
                        it.printStackTrace()
...
fun PipelineContext<Unit, ApplicationCall>.getUserId() =
    call.principal<UserIdPrincipal>("auth-session") ?: call.principal<UserIdPrincipal>("auth-bearer")
...
fun PipelineContext<Unit, ApplicationCall>.getWalletId() = runCatching {
...
fun PipelineContext<Unit, ApplicationCall>.ensurePermissionsForWallet(
    ...
    val permissions = transaction {
        (AccountWalletMappings.selectAll().where {
            (AccountWalletMappings.tenant eq "") and (AccountWalletMappings.accountId eq userId) and (AccountWalletMappings.wallet eq walletId)
        } // FIXME -> TENANT HERE
            .firstOrNull()
            ?: throw ForbiddenException("This account does not have access to the specified wallet."))[AccountWalletMappings.permissions]
    }
