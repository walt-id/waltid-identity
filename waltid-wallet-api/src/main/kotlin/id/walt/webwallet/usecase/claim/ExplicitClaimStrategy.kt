...
class ExplicitClaimStrategy(
    ...
    suspend fun claimAuthorize(
        tenant: String, account: UUID, wallet: UUID, did: String, offer: String, pending: Boolean = true
    ): HttpResponse = issuanceService.useOfferRequestAuthorize(
        offer = offer,
        credentialWallet = SSIKit2WalletService.getCredentialWallet(did),
        clientId = SSIKit2WalletService.testCIClientConfig.clientID,
        walletId = wallet
    ...
    suspend fun exhangeCodeForToken(
        tenant: String, account: UUID, wallet: UUID, did: String, code: String, pending: Boolean = true
    ): List<WalletCredential> = issuanceService.useCodeForToken(
        code = code,
        credentialWallet = SSIKit2WalletService.getCredentialWallet(did),
        clientId = SSIKit2WalletService.testCIClientConfig.clientID,
        walletId = wallet
