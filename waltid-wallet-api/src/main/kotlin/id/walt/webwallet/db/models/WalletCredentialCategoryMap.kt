...
object WalletCredentialCategoryMap : Table("credential_category") {
    ...
    val category = reference("category", WalletCategory, onDelete = ReferenceOption.CASCADE)

    override val primaryKey = PrimaryKey(wallet, credential, category)

    init {
        foreignKey(wallet, credential, target = WalletCredentials.primaryKey, onDelete = ReferenceOption.CASCADE)
    }
}