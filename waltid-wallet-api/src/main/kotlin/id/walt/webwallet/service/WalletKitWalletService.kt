...
class WalletKitWalletService(tenant: String, accountId: UUID, walletId: UUID) :
    ...
    private val userEmail: String by lazy {
        transaction {
            Accounts.selectAll().where { Accounts.id eq this@WalletKitWalletService.walletId }.single()[Accounts.email]
    ...
    private suspend inline fun <reified T> authenticatedJsonPost(
        path: String, body: T, crossinline block: HttpRequestBuilder.() -> Unit = {}
    ): HttpResponse = http.post(path) {
        bearerAuth(token.value)
        contentType(ContentType.Application.Json)
        setBody(body)
        block.invoke(this)
    ...
    private suspend fun auth(): String = (http.post("/api/auth/login") {
        setBody(AuthRequest(userEmail, userEmail))
        contentType(ContentType.Application.Json)
    }.body<JsonObject>()["token"] ?: throw IllegalStateException("Could not login: $userEmail")).jsonPrimitive.content
...
//                delete = false,
    ...
    override suspend fun getCredential(credentialId: String) =
        listCredentials(CredentialFilterObject.default).first { it.parsedDocument?.get("id")?.jsonPrimitive?.content == credentialId }
            .let {
                WalletCredential(
                    wallet = walletId,
                    id = credentialId,
...
//                delete = false,
    ...
    override fun matchCredentialsByPresentationDefinition(presentationDefinition: PresentationDefinition): List<WalletCredential> =
        throw NotImplementedError("")/*prettyJson.encodeToString(*/
    //)
    ...
    override suspend fun usePresentationRequest(parameter: PresentationRequestParameter): Result<String?> {
        ...
        val sessionId = authenticatedJsonPost(
            "/api/wallet/presentation/startPresentation", mapOf("oidcUri" to parameter.request)
        ).bodyAsText()
        ...
        val redirectUriResp = http.submitForm(redirectUri, formParameters = parameters {
    ...
    override suspend fun listDids() = authenticatedJsonGet("/api/wallet/did/list").body<List<WalletDid>>()

    override suspend fun loadDid(did: String) = authenticatedJsonGet("/api/wallet/did/$did").body<JsonObject>()
    ...
    override suspend fun exportKey(alias: String, format: String, private: Boolean): String = authenticatedJsonPost(
    ...
    override suspend fun listKeys() =
        authenticatedJsonGet("/api/wallet/keys/list").body<JsonObject>()["list"]!!.jsonArray.map {
            Json.decodeFromJsonElement<SingleKeyResponse>(it)
        }
    ...
    override suspend fun importKey(jwkOrPem: String) =
        authenticatedJsonPost("/api/wallet/keys/import", body = jwkOrPem).body<String>()
...
//fun infoAboutOfferRequest
    ...
    override fun getHistory(limit: Int, offset: Long): List<WalletOperationHistory> = transaction {
        WalletOperationHistories.selectAll()
            .where { (WalletOperationHistories.tenant eq tenant) and (WalletOperationHistories.accountId eq walletId) }
            .orderBy(WalletOperationHistories.timestamp).limit(10).map { WalletOperationHistory(it) }
    }
