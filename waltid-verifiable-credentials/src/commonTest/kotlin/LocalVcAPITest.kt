...
class LocalVcApiTest {
    ...
    suspend private fun testCreateSignCredential(keyType: KeyType) {
        println("-- testCreateSignCredential: $keyType --")

        println("Generating LocalKey for: $keyType")
        val key = LocalKey.generate(keyType)

        println("Create and Sign VC using Key ${key}...\n")
    ...
    suspend fun testCheqd(key: LocalKey) {
        println("- Test did:cheqd with $key... -")
