...
object RequestSigningCryptoProvider: JWTCryptoProvider {
  val signingKey: RSAKey? = ConfigManager.getConfig<OIDCVerifierServiceConfig>().requestSigningKeyFile?.let { runBlocking { RSAKey.parseFromPEMEncodedObjects(FileReader(it).readText()).toRSAKey() } }
  ...
  override fun sign(payload: JsonObject, keyID: String?, typ: String): String {
    ...
    return SignedJWT(
      JWSHeader.Builder(JWSAlgorithm.RS256).keyID(signingKey.keyID).type(JOSEObjectType.JWT).x509CertChain(
