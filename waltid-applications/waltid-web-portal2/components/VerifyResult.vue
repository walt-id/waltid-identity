...
const verifiedClaims = computed(() => {
  if (!props.session.sse.isTerminal.value || props.session.sse.status.value !== 'SUCCESSFUL') return null
  const last = [...props.session.sse.events.value].reverse().find(ev => ev.parsed)
  ...
  <div class="grid gap-4">
    <template v-if="session.result.value">
      ...
      <div v-if="verifiedClaims" class="grid gap-2">
        ...
        <pre class="log-box max-h-64">{{ JSON.stringify(verifiedClaims, null, 2) }}</pre>
      </div>
    </template>

    <div v-else class="flex items-center justify-center min-h-[200px] text-sm text-[--color-text-muted] text-center">
