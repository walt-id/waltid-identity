...
function applySecurityOverridesToJson() {
  ...
  const runtimeOverrides = (
    payload.runtimeOverrides && typeof payload.runtimeOverrides === 'object' && !Array.isArray(payload.runtimeOverrides)
  ...
  try {
    ...
    optionsError.value = e instanceof Error ? e.message : 'Invalid issuer certificate.'
    return null
  }
    ...
    <details class="group rounded-xl border border-[--color-border] bg-white">
      <summary class="cursor-pointer list-none p-4">
        <div class="flex items-center justify-between gap-4">
          <div>
            <span class="text-base font-semibold">Offer security overrides</span>
            <span class="block text-sm text-[--color-text-muted] mt-1">
              These controls override <code>runtimeOverrides.issuerKey</code> and <code>runtimeOverrides.x5Chain</code> before creating the offer.
            </span>
          </div>
      ...
      <div class="grid gap-4 px-4 pb-4">
        ...
        <div class="grid gap-2">
          ...
          <button type="button" class="btn btn-secondary justify-self-start" @click="addX5ChainInput">
            Add certificate
          </button>
        </div>
