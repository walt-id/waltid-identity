...
watch(
  selectedOption,
  (option) => {
    credentialDataJson.value = JSON.stringify(option.defaultCredentialData, null, 2)
...
async function submit() {
  ...
  if (credentialDataJson.value.trim()) {
    try {
      ...
      parseError.value = e instanceof Error ? e.message : 'Credential data must be valid JSON.'
      return
    }
  }
  ...
  <div class="grid gap-5">
    <section>
      ...
      <p class="text-sm text-[--color-text-muted] mb-3">
        Pick a credential type and customise only the data that should appear in the credential.
      </p>
        ...
        >
          <div class="flex items-start justify-between gap-3">
            <div>
              ...
              <p class="text-xs text-[--color-text-muted] mt-1">{{ option.profileId }}</p>
            </div>
          </div>
    ...
    <section>
      ...
      <div class="inline-flex rounded-lg border border-[--color-border-strong] bg-white p-1">
        <button
          ...
          :class="authMethod === 'PRE_AUTHORIZED' ? 'bg-slate-900 text-white' : 'text-[--color-text-muted] hover:text-[--color-text]'"
        ...
        <button
          type="button"
          class="px-3 py-1.5 text-sm font-medium rounded-md transition-colors"
          :class="authMethod === 'AUTHORIZED' ? 'bg-slate-900 text-white' : 'text-[--color-text-muted] hover:text-[--color-text]'"
      ...
      <p class="text-xs text-[--color-text-muted] mt-2">
        Pre-authorized is simplest for demos. Authorized uses the wallet authorization flow.
      </p>
    </section>
    ...
    <section>
      ...
      <p class="text-xs text-[--color-text-muted] mb-2">
        Edit the data below. It will be applied as <code>runtimeOverrides.credentialData</code> for the selected profile.
      </p>
      <textarea
        ...
        spellcheck="false"
      />
      <p v-if="parseError" class="text-sm text-red-600 mt-2">{{ parseError }}</p>
    </section>
