  ...
  <div class="grid gap-4">
    <template v-if="session.result.value">
      <QrDisplay
        ...
        :label="session.result.value.credentialOffer"
      />

      <div v-if="session.result.value.txCodeValue" class="p-3 rounded-lg bg-amber-50 border border-amber-200 text-sm">
        <span class="font-semibold text-amber-800">PIN: </span>
        <code class="font-mono text-amber-900">{{ session.result.value.txCodeValue }}</code>
      </div>
    </template>
