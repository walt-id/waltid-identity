...
function eventColor(ev: SseEvent): string {
  ...
  if (['FAILED', 'UNSUCCESSFUL', 'REJECTED', 'EXPIRED'].includes(upper)) return 'text-red-400'
...
<template>
  <div class="grid gap-2">
    <div class="flex items-center justify-between">
      ...
      <span v-if="status" class="text-xs font-semibold" :class="status === 'SUCCESSFUL' ? 'text-green-600' : 'text-red-600'">
        {{ status }}
      </span>
    </div>

    <div ref="container" class="log-box min-h-[160px] max-h-[320px]">
      <template v-if="events.length === 0"><span class="text-[--color-text-muted]">(waiting for events…)</span></template>
    ...
    <div v-if="isTerminal && status === 'SUCCESSFUL'" class="p-3 rounded-lg bg-green-50 border border-green-200 text-sm text-green-800 font-medium">
      Completed successfully
    </div>
    <div v-else-if="isTerminal" class="p-3 rounded-lg bg-red-50 border border-red-200 text-sm text-red-800 font-medium">
      {{ status }}
    </div>
  </div>
</template>
