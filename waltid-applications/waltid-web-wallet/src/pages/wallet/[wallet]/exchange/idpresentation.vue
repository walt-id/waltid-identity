  ...
  <div>
    <PageHeader>
      <template v-slot:title>
        <div class="ml-3">
          <h1 class="text-2xl font-bold leading-7 text-gray-900 sm:truncate sm:leading-9">Present ID Token</h1>
          <p>
    ...
    <CenterMain>
      ...
      <h1 class="text-2xl font-semibold mb-2">ID Token Presentation</h1>
      ...
      <fieldset class="mt-3">
        <legend class="text-base font-semibold leading-6 text-gray-900">Present your ID Token with your DID</legend>
      <div><br></div>
...
<!--        </div>-->
      ...
      <Disclosure>
        <DisclosureButton class="py-2">
          <ButtonsWaltButton class="bg-gray-400 text-white">View ID Token Request JSON</ButtonsWaltButton>
        </DisclosureButton>
...
async function resolveIdRequest(request) {
  try {
    console.log("RESOLVING request", request);
    // let reqDecoded = decodeURI(request)
    const requestUrl = new URL(request);
    const requestParams = requestUrl.searchParams;
    const host = new URL(requestParams.get("response_uri") ?? requestParams.get("redirect_uri") ?? "").host;
...
const requestParams = await resolveIdRequest(decodeRequest(query.request));
console.log("Decoded request: " + requestParams);

const presentationDefinition = requestParams
const verifierHost = new URL(requestParams.get("response_uri") ?? requestParams.get("redirect_uri") ?? "").host;
...
async function acceptPresentation() {
  ...
  const response = await fetch(`/wallet-api/wallet/${currentWallet.value}/exchange/useIdTokenRequest?${req.requestParams}`, {
    method: "GET",
  ...
  if (response.ok) {
    ...
    if (parsedResponse.redirectUri) {
      ...
      window.alert("ID Token Presentation successful");
