        ...
        <div v-else>
            ...
            <div v-if="didDoc">
                ...
                <div class="p-3 shadow mt-3">
                    ...
                    <qrcode-vue v-if="didDoc && JSON.stringify(didDoc).length <= 4296" :value="JSON.stringify(didDoc)"
                        level="L" size="300"></qrcode-vue>
                    <p v-else-if="didDoc && JSON.stringify(didDoc).length">
                        Unfortunately, this DID document is too big to be viewable as QR code (DID document size is {{
                            didDoc.length }} characters, but the maximum a QR code can hold is 4296).
