<template>
    <CenterMain>
        ...
        <div v-if="groupedHistoryElements.size > 0" aria-label="Credential list"
            ...
            <div v-for="group in groupedHistoryElements.keys()" :key="group.id" class="relative">
                ...
                <ul class="divide-y divide-gray-100 overflow-x-clip" role="list">
                    <li v-for="element in groupedHistoryElements.get(group)" :key="element"
                        ...
                        <div class="min-w-0">
                            <div class="flex items-start gap-x-3">
                                ...
                                <div v-else-if="element.entry.operation === 'usePresentationRequest'">
                                    <p class="text-sm font-semibold leading-6 text-gray-900">Used presentation request
                                    </p>
                                    <p class="text-xs overflow-x-scroll w-1/3 h-5">
                                        <code>{{ JSON.stringify(element.entry.data) }}</code>
                                    </p>
                                ...
                                <div v-else>
                                    <p class="text-sm font-semibold leading-6 text-gray-900">{{ element.entry.operation
                                        }}</p>
                                    <p class="text-xs overflow-x-scroll w-1/3 h-5">
                                        <code>{{ JSON.stringify(element.entry.data) }}</code>
                                    </p>
