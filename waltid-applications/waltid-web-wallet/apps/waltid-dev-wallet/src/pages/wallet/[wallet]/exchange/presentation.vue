<template>
    <div>
        ...
        <CenterMain>
            ...
            <LoadingIndicator v-if="immediateAccept" class="my-6 mb-12 w-full"> Presenting credential(s)...
            </LoadingIndicator>
                    ...
                    <div v-for="(credential, credentialIdx) in matchedCredentials" v-else :key="credentialIdx"
                        ...
                        <div class="min-w-0 flex-1 text-sm leading-6">
                            <div class="font-medium select-none text-gray-900">
                                <label :for="`credential-${credential.id}`">
                                    <VerifiableCredentialCard
                                        ...
                                        :credential="credential" :show-id="true" />
                            ...
                            <div v-if="credential.disclosures && selection[credential.id]"
                                ...
                                <fieldset>
                                    <legend class="text-base font-semibold leading-6 text-gray-900">Selectively
                                        disclosable
                                        attributes
                                    </legend>
                                            ...
                                            <div class="min-w-0 flex-1 text-sm leading-6">
                                                <label :for="`disclosure-${credential.id}-${disclosure[0]}`"
                                                    ...
                                                    <div class="md:flex text-gray-500 mb-3 md:mb-1">
                                                        <div class="min-w-[19vw]">{{ disclosureIdx + 1 }}. <span
                                                                class="font-semibold">{{ disclosure[1] }}</span></div>
                                                        <div class="grow-0">
                                                            {{ disclosure[2] }}
                                                        </div>
                                                    </div>
            ...
            <Disclosure>
                <DisclosureButton class="py-2">
                    <ButtonsWaltButton class="bg-gray-400 text-white">View presentation definition JSON
                    </ButtonsWaltButton>
                </DisclosureButton>
...
const { currentWallet, verifierHost, presentationDefinition, matchedCredentials, selectedCredentialIds, selection, addDisclosure, removeDisclosure, acceptPresentation, failed, failMessage } = await usePresentation(query);
