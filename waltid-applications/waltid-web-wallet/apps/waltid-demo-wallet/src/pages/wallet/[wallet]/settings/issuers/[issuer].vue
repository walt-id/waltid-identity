<template>
    ...
    <div class="hidden sm:block p-8">
        ...
        <ul
            ...
            <li v-for="credential in issuerCredentials?.credentials.filter(c => c.format == credentialType)"
                :key="credential.id" class="col-span-1 divide-y divide-gray-200 rounded-2xl bg-white shadow transform hover:scale-105
                        ...
                        <div class="mb-8">
                            <div class="text-2xl font-bold bold text-white">
                                {{ credential.id.split('_')[0].length > 20 ?
                                    credential.id.split('_')[0].substring(0, 20) + '...' : credential.id.split('_')[0]
                                }}
                            </div>
                        </div>

                        <div class="sm:mt-18">
                            <div :class="{ 'text-white': issuer, 'text-[#0573f000]': !issuer }">Issuer</div>
    ...
    <CenterMain class="sm:hidden">
        ...
        <div class="mt-8">
            ...
            <p v-if="issuerCredentials?.credentials.filter(c => c.format == credentialType).length == 0"
...
const { pending, data: issuerCredentials, error, refresh } = useLazyFetch<IssuerCredentials>(`/wallet-api/wallet/${currentWallet.value}/issuers/${issuer}/credentials`);
