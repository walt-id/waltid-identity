        ...
        <div v-else>
            ...
            <div class="hidden sm:block px-4 py-6 bg-white rounded-xl">
                ...
                <div v-if="credential?.format === 'mso_mdoc'">
                    ...
                    <div v-for="elem in
                        ...
                        <div class="flex mt-3">
                            ...
                            <div class="text-gray-500 font-bold w-2xl">{{ elem.elementValue }}</div>
                        </div>
                    </div>
                </div>

                <hr class="w-full border-gray-200 mb-2 mt-8" v-if="issuerName || issuerDid" />
                <div class="text-gray-500 font-bold mt-4 mb-8" v-if="issuerName || issuerDid">Issuer</div>
                ...
                <div class="flex mt-2 mb-8" v-if="issuerDid">
                    ...
                    <div class="text-gray-500 font-bold w-2xl overflow-scroll">{{ issuerDid }}</div>
                </div>
                ...
                <div class="flex justify-between my-6">
                    ...
                    <div class="text-gray-500" v-if="issuanceDate">Issued {{ issuanceDate?.replace(/-/g, ".") }}</div>
            ...
            <div class="px-4 py-6 shadow-sm bg-white rounded-xl sm:hidden"
                ...
                <div v-if="credential?.format === 'mso_mdoc'">
                    ...
                    <div v-for="(elem, index) in
                        ...
                        <hr v-if="index !== jwtJson?.issuerSigned?.nameSpaces[Object.keys(jwtJson?.issuerSigned?.nameSpaces)[0]].length - 1"
                            class="w-full border-gray-200 my-2" />
                    </div>
                </div>
            </div>
...
import { type WalletCredential, useCredential } from "@waltid-web-wallet/composables/credential.ts";
...
const { jwtJson, disclosures, issuerName, issuerDid, issuanceDate, expirationDate } = useCredential(credential);

const credentialManifest = computedAsync(async () => {
    if (jwtJson.value) {
        const { data } = await useFetch(`${runtimeConfig.public.credentialsRepositoryUrl}/api/manifest/${jwtJson.value?.type[jwtJson.value?.type.length - 1]}`, {
