...
/// removed. This is safe precisely because it is authoritative: the wallet publishes one registration
/// per presentable mdoc credential it holds, this integration is the only writer of Apple's store for
/// this provider, and a projection that could not be read never reaches here - ``desiredRegistrations(from:)``
/// fails closed first. Removing a registration stops the platform offering a document, which is exactly
/// the intent when the wallet no longer holds it.
///
/// - Parameters:
...
/// removes every registration absent from `desired`, so reading "I could not find out what the wallet
/// holds" as an empty desired set would unregister documents the wallet can still present.
///
